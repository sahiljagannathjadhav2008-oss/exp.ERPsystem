...
@Entity(
    ...
    indices = [
        ...
        // Covers the warehouseId foreign key on its own: Room flags an FK
        // column as "missing an index" unless it is the LEADING column of
        // at least one index. The composite unique index above has
        // productId first, so it does not satisfy that for warehouseId —
        // this second index does, and also speeds up any query/JOIN or
        // cascading FK action that looks stock up by warehouse alone.
...
@Entity(
    ...
    indices = [
        ...
        // Same reasoning as StockEntity: warehouseId needs to lead an
        // index of its own to count as FK-covered.
        Index(value = ["warehouseId"]),
...
@Entity(
    ...
    indices = [
        Index(value = ["productId", "warehouseId"]),
        // Same reasoning as StockEntity/StockMovementEntity.
        Index(value = ["warehouseId"]),
